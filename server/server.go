...
func (s *userServiceServer) GetUserDetails(ctx context.Context, req *pb.UserDetailsRequest) (*pb.UserDetailsResponse, error) {
	var name string
	var age int32
	err := s.db.QueryRow("SELECT username, age FROM users WHERE token = $1", req.Token).Scan(&name, &age)
	if err != nil {
...
func main() {
	db, err := sql.Open("postgres", "postgresql://postgres:12345@localhost:5432/GoGrpc?sslmode=disable")
	if err != nil {
